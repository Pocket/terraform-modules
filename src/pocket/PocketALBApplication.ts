...
import {CloudfrontDistribution, Route53Record} from "../../.gen/providers/aws";
...
export class PocketALBApplication extends Resource {
    ...
    constructor(scope: Construct, name: string, config: PocketALBApplicationProps) {
        super(scope, name);
        ...
        this.alb = new ApplicationLoadBalancer(this, `${name}_application_load_balancer`, {
            vpcId: pocketVPC.vpc.id!,
            prefix: config.prefix,
            alb6CharacterPrefix: config.alb6CharacterPrefix,
            subnetIds: config.internal ? pocketVPC.privateSubnetIds : pocketVPC.publicSubnetIds,
        ...
        if (config.cdn) {
            ...
            const cdnCertificate = new ApplicationCertificate(this, `${name}_cdn_certificate`, {
                zoneId: this.baseDNS.zoneId,
            ...
            //Create the CDN
            const cdn = new CloudfrontDistribution(this, `${name}_cloudfront_distribution`, {
